...
from database import (
    ...
    get_scheduled_broadcasts, cancel_broadcast,
)
from keyboards import (
    ...
    scheduled_broadcasts_keyboard, broadcast_cancel_confirm_keyboard,
)
...
async def broadcast_action(callback: CallbackQuery, state: FSMContext):
    ...
    if action == "history":
        ...
        for bc in history:
            if bc["status"] == "sent":
                status_icon = "✅"
            elif bc["status"] == "cancelled":
                status_icon = "🚫"
            else:
                status_icon = "🕐"
            text += (
                f"{status_icon} {(bc['created_at'] or '')[:16]}\n"
                f"   Отправлено: {bc['sent_count']} | Ошибок: {bc['fail_count']}\n"
                f"   {(bc['text'] or '')[:60]}{'...' if len(bc['text'] or '') > 60 else ''}\n\n"
            )
        await callback.message.edit_text(text, parse_mode="HTML")
        await callback.answer()
        return
    ...
    if action == "pending":
        ...
        if not broadcasts:
            ...
            return
        await callback.message.edit_text(
            "🕐 <b>Запланированные рассылки:</b>\n\nНажмите на рассылку чтобы отменить:",
            parse_mode="HTML",
...
async def broadcast_cancel_view(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.answer("Ошибка", show_alert=True)
        return
    history = get_broadcast_history(100)
    bc = next((b for b in history if b["id"] == bc_id), None)
    if not bc or bc["status"] != "pending":
        await callback.answer("Рассылка уже отправлена или отменена", show_alert=True)
        return
